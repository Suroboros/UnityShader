	...
	SubShader {
		Pass{
			...
			fixed4 fragShader (FragmentInputType input) : SV_Target
			{
				...
				// Ambient
				float3 ambient = _Ka * lightColor * _MaterialColor.rgb * textureColor.rgb;
				// Diffuse
				float3 lightDirection = normalize(_WorldSpaceLightPos0.xyz);
				float lightIntensity = max(dot(input.normal,lightDirection), 0);
				float3 diffuse = _MaterialColor.rgb * lightColor * lightIntensity * textureColor.rgb;
				// Specular
		...
		Pass{
			...
			fixed4 fragShader (FragmentInputType input) : SV_Target
			{
				...
				float lightIntensity = max(dot(input.normal,lightDirection), 0);
				float3 diffuse = attenuation * textureColor.rgb * _MaterialColor.rgb * lightColor * lightIntensity;
				// Specular
				float3 viewDirection = normalize(_WorldSpaceCameraPos-input.posInWorld.xyz);
				float3 halfDirection = normalize(lightDirection + viewDirection);
				float specularLight = pow(max(dot(halfDirection,input.normal) , 0), _Shininess);
				if(lightIntensity <= 0) specularLight = 0;
				float3 specular = attenuation * _SpecularColor.rgb * lightColor * (1.0 - textureColor.a) * specularLight;
				// Final color
